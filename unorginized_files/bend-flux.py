...
flux_freqs = []
straight_tran_flux = []
for freq in np.arange((fcen-(df/2)),(fcen+(df/2)+((df)/(nfreq-1))),((df)/(nfreq-1))):    # pulse width (in frequency)
    sources = [mp.Source(mp.ContinuousSource(frequency=freq,end_time=1),
                        component=mp.Ez,
 ...
 # number of frequencies at which to compute flux
    ...
    straight_tran_flux_instant = mp.get_fluxes(tran)
    straight_tran_flux = np.append(straight_tran_flux,straight_tran_flux_instant[0])
    sim.reset_meep()
    ...
    bend_refl_flux_instant = mp.get_fluxes(refl)
    bend_tran_flux_instant = mp.get_fluxes(tran)
    bend_refl_flux = np.append(bend_refl_flux,bend_refl_flux_instant[0])
    bend_tran_flux = np.append(bend_tran_flux,bend_tran_flux_instant[0])
    flux_freqs = np.append(flux_freqs,mp.get_flux_freqs(refl)[0])
